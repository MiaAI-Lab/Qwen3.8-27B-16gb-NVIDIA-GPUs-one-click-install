...
**More than one NVIDIA GPU:** CUDA's default device order is fastest-first, which is *not* nvidia-smi order. On a 5080 + 5090 the setup page used to plan for the 5080 (smi GPU 0, 16 GB) and then load on the 5090. The setup page now shows a **Which GPU** toggle whenever nvidia-smi sees two or more cards; picking one writes `CUDA_DEVICE_ORDER=PCI_BUS_ID` and `CUDA_VISIBLE_DEVICES=<index>` into `.env`. `start.bat` copies those into the server process (Windows never `source`s `.env`). Console / Linux: `start.sh` exports them after sourcing `.env`. Set the two lines by hand and restart if you would rather not use the page.
